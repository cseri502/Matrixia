    ...
    <div class="relative mx-auto min-h-screen max-w-7xl flex flex-col items-center">
        <div class="text-center mb-8">
            ...
            <p class="mt-4 text-lg text-gray-600 dark:text-gray-300">
                Easily calculate the adjoint (or adjugate) of your matrix.
            </p>
        </div>

        <LoadingSpinner v-if="isLoading" />
        
        <MatrixInput v-show="!isLoading" :isTwoInputs="false" apiUrl="/matrixia/adjugate" @setLoading="setLoadingState"
            @taskResult="handleResult" class="mb-4" />
        <div v-if="!isLoading && adjugate"
            ...
            <p class="mt-2 text-lg text-gray-700 dark:text-gray-300">
                The adjugate matrix is shown below:
            </p>
            <div class="bg-white dark:bg-gray-700 p-4 rounded-lg shadow-sm mt-4">
                <table class="min-w-full border-separate border-spacing-2 border-none">
                    <thead>
                        <tr>
                            <th class="w-10"></th>
                            <th v-for="colIndex in adjugate.length" :key="'header-' + colIndex"
                    ...
                    <tbody>
                        <tr v-for="(row, rowIndex) in adjugate" :key="'row-' + rowIndex">
...
const adjugate = ref<number[][] | null>(null);

const handleResult = (result:  { adjugate: number[][] }) => {
    adjugate.value = result?.adjugate;
};
