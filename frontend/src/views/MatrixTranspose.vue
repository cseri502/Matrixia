    ...
    <div class="relative mx-auto min-h-screen max-w-7xl flex flex-col items-center">
        <div class="text-center mb-8">
            ...
            <p class="mt-4 text-lg text-gray-600 dark:text-gray-300">
                Easily calculate the transposed version of your matrix.
            </p>
        </div>

        <LoadingSpinner v-if="isLoading" />
        
        <MatrixInput v-show="!isLoading" :isTwoInputs="true" apiUrl="/matrixia/transpose" @setLoading="setLoadingState"
            @taskResult="handleResult" class="mb-4" />
        <div v-if="!isLoading && transpose"
            ...
            <div class="bg-white dark:bg-gray-700 p-4 rounded-lg shadow-sm mt-4">
                <table class="min-w-full border-separate border-spacing-2 border-none">
                    <thead>
                        <tr>
                            <th class="w-10"></th>
                            <th v-for="colIndex in transpose.length" :key="'header-' + colIndex"
                    ...
                    <tbody>
                        <tr v-for="(row, rowIndex) in transpose" :key="'row-' + rowIndex">
...
const transpose = ref<number[][] | null>(null);

const handleResult = (result: { transpose: number[][] }) => {
    transpose.value = result?.transpose;
};
